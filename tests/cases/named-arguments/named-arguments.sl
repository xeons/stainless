...
class Rect
{
    ...
    public String Text() => $"{Left},{Top} {Right},{Bottom}";
}
...
String Show(String value, bool quoted) => quoted ? "\"" + value + "\"" : value;
...
public int Main()
{
    ...
    if (TryHalve(9, out var half, evenOnly: false))
        Console.WriteLine($"half   {half}");
    if (!TryHalve(9, out var nothing, evenOnly: true))
        Console.WriteLine($"refused {nothing}");
    return 0;
}
