...
{
    if (settings.TryGetValue(name) is Some found)
...
int Main()
{
    ...
    Console.WriteLine("told apart " +
        (settings.TryGetValue("retries").HasValue ? "set" : "unset") + " " +
        (settings.TryGetValue("absent").HasValue ? "set" : "unset"));

    // The combinators come free, Optional being an ordinary variant.
    Console.WriteLine("valueOr " + N((long)settings.TryGetValue("absent").GetValueOrDefault(-1)));
    Console.WriteLine("empty " + (settings.TryGetValue("absent").IsEmpty ? "y" : "n"));
    ...
    Console.WriteLine("ref " + names.TryGetValue(1).GetValueOrDefault("?") + " " +
        names.TryGetValue(2).GetValueOrDefault("?"));
    ...
    Console.WriteLine("sorted " + N((long)prices.TryGetValue("apple").GetValueOrDefault(-1)) + " " +
        N((long)prices.TryGetValue("pear").GetValueOrDefault(-1)));
