...
int Main()
{
    ...
    Console.WriteLine("lookup " + seen.GetOr(new Point(1, 1), "<missing>"));
    Console.WriteLine("missing " + seen.GetOr(new Point(9, 9), "<missing>"));
    Console.WriteLine("count " + Text.FromInteger(seen.Count));

    // Setting the same value twice replaces rather than adds.
    seen.Set(new Point(1, 1), "again");
    Console.WriteLine("replaced " + seen.GetOr(new Point(1, 1), "<missing>")
