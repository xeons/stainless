...
class Node {
    ...
    public Value Get() { return Payload; }
}

// A field: the case this exists for.
String OfField(Node node) {
    if (node.Payload is Number n) { return "number " + Text.FromDouble(n.Held); }
    if (node.Payload is Bool b) { return "bool " + Text.FromBool(b.Held); }
    if (node.Payload is Pair p) {
        return "pair " + Text.FromInteger((long)p.Left) + "," + Text.FromInteger((long)p.Right);
    }
    if (node.Payload is Null) { return "null"; }
    return "?";
}
...
String OfCall(Node node) {
    if (node.Get() is Number n) { return "call " + Text.FromDouble(n.Held); }
    return "call none";
}
...
String OfParameter(Value value) {
    if (value is Pair p) { return "local " + Text.FromInteger((long)(p.Left + p.Right)); }
    if (value is Number) { return "local " + Text.FromDouble(value.Held); }
    return "local none";
}

// An else, and an else-if chain over the same field.
String Chain(Node node) {
    if (node.Payload is Bool b) { return "chain bool " + Text.FromBool(b.Held); }
    else if (node.Payload is Number n) { return "chain number " + Text.FromDouble(n.Held); }
    else { return "chain other"; }
}

// The binding is a value like any other, so a case carrying two fields names
// both of them.
int Sum(Node node) {
    if (node.Payload is Pair p) { return p.Left + p.Right; }
    return 0;
}
...
String Speak(Animal animal) {
    if (animal is Dog dog) { return "dog says " + dog.Says(); }
    return "animal says " + animal.Says();
}

String Deep(Animal? animal) {
    if (animal is Puppy p) { return "puppy " + p.Says(); }
    return "not a puppy";
}
...
class Link {
    public int Value;
    public Link? Next;
    public Link(int value) { Value = value; Next = null; }
}
...
public int Main() {
    ...
    for (nuint i = 0u; i < boxes.Count(); i = i + 1u) {
        if (boxes.At(i).Get() is Number n) { builder.AppendDouble(n.Held); }
        else { builder.Append("."); }
    }
