...
export "C" nuint WordSize() => sizeof(Word);
export "C" nuint WideSize() => sizeof(Wide);
export "C" nuint TaggedSize() => sizeof(Tagged);

// Written on one side, read on the other: the members have to overlap the same
// way in both or these come back wrong.
export "C" uint ReadUnsigned(Word word) => word.Unsigned;
export "C" int ReadBits(Word word) => word.Signed;
export "C" int ReadPairB(Wide wide) => wide.P.B;
...
{
    if (tagged.Which == Kind.AsInt)
        return tagged.Value.Signed;
    return -1;
}
