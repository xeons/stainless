...
public class Counter
{
    ...
    ~Counter() { _tally.Destroyed = _tally.Destroyed + 1; }

    public void Bump() => _count = _count + Step;
    public int Total() => _count;
    public String Describe() => Label + ":" + Text.FromInteger(_count);
}
...
public class Note
{
    public virtual String Body() => "note";
    public virtual int Weight() => 1;
}

public class Urgent : Note
{
    public override String Body() => "urgent";
}

/// Made on this side, held as the base on the other.
public Note MakeUrgent() => new Urgent();

public int KindOf(Kind kind) => (int)kind;
