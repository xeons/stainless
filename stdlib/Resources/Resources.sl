...
///
/// @param type  which `RT_` kind it was filed as
/// @param id    the number the script filed the resource under
public bool Exists(ResourceType type, int id) =>
    FindBytes((int)type, id, null) != null;
...
/// @param id    the number the script filed the resource under
public uint GetSize(ResourceType type, int id)
{
    uint count = 0u;
    FindBytes((int)type, id, &count);
    return count;
}
...
public byte* GetPointer(ResourceType type, int id, uint* byteCount)
{
    return FindBytes((int)type, id, byteCount);
}
...
public byte[] GetBytes(ResourceType type, int id)
{
    uint count = 0u;
    byte* at = FindBytes((int)type, id, &count);
...
{
    uint count = 0u;
    byte* block = FindBytes((int)ResourceType.StringTable, (int)(id / 16u) + 1, &count);
    if (block == null)
        return "";
...
{
    var stored = GetBytes(ResourceType.Bitmap, id);
