...
/// @see Convert.FromHexString
public String ToHexString(byte[] data, bool upper)
{
    var built = new StringBuilder();
...
/// @see Convert.ToHexString
public Result<byte[], ConvertError> FromHexString(String text)
{
    nuint size = text.ByteLength();
    if (size % 2 != 0)
        return Fail(ConvertError.Malformed);
...
/// @see Convert.ToBase64String
/// @see Convert.ToBase64Url
public Result<byte[], ConvertError> FromBase64String(String text)
{
    nuint size = text.ByteLength();
    var bytes = text.ToPointer();
...
{
    return ToBase64String(text.ToBytes());
}
