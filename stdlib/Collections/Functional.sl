...
// `Comparison` -- are declared in `Standard`, and need no import to reach.
...
/// @typeparam TAccumulate  what is carried along and answered, taken from the seed
public TAccumulate Aggregate<T, TAccumulate>(
    IEnumerable<T> items, TAccumulate seed, Fold<TAccumulate, T> combine)
{
    var total = seed;
    foreach (var item in items)
        total = combine(total, item);
