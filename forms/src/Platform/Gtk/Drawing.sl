...
public class GtkBitmapBackend : IBitmapBackend {
    ...
    public GtkBitmapBackend(gpointer image) { pixbuf = image; }
    ...
    ~GtkBitmapBackend() {
        if (pixbuf != null) { g_object_unref(pixbuf); pixbuf = null; }
    }

    public int Width()  { return gdk_pixbuf_get_width((GdkPixbuf*)pixbuf); }
    public int Height() { return gdk_pixbuf_get_height((GdkPixbuf*)pixbuf); }

    public nuint Handle() { return (nuint)pixbuf; }

    /// The pixbuf itself, for the image list and the toolbar, which need the
    /// object rather than a number.
    public gpointer Pixbuf() { return pixbuf; }
}
...
public class GtkImageListBackend : IImageListBackend {
    List<gpointer> pictures;
    FSize extent;

    public GtkImageListBackend(FSize imageSize) {
        pictures = new List<gpointer>();
        extent = imageSize;
    }

    ~GtkImageListBackend() {
        for (nuint i = 0u; i < pictures.Count(); i++) { g_object_unref(pictures[i]); }
        pictures.Clear();
    }
    ...
    public int Add(IBitmapBackend picture) {
        ...
        // came from.
        if (gdk_pixbuf_get_width((GdkPixbuf*)source) != extent.Width ||
            gdk_pixbuf_get_height((GdkPixbuf*)source) != extent.Height) {
            scaled = gdk_pixbuf_scale_simple((GdkPixbuf*)source, extent.Width,
                                             extent.Height, GDK_INTERP_BILINEAR);
        } else {
            g_object_ref(source);
        }

        pictures.Add(scaled);
        return (int)pictures.Count() - 1;
    }

    public int Count() { return (int)pictures.Count(); }
    public FSize ImageSize() { return extent; }
    ...
    public gpointer Pixbuf(int index) {
        if (index < 0 || (nuint)index >= pictures.Count()) { return null; }
        return pictures[(nuint)index];
    }
}
...
public class GtkGraphicsBackend : IGraphicsBackend {
    ...
    public GtkGraphicsBackend(gpointer context) { cairo = context; }
    ...
    public int PushLayer(Rectangle bounds) {
        cairo_save(cairo);
        cairo_rectangle(cairo, (double)bounds.X, (double)bounds.Y,
                        (double)bounds.Width, (double)bounds.Height);
        cairo_clip(cairo);
        cairo_translate(cairo, (double)bounds.X, (double)bounds.Y);
        depth = depth + 1;
        return depth;
    }
    ...
    void Source(Color colour) {
        cairo_set_source_rgb(cairo, (double)(int)colour.R / 255.0,
    ...
    double Ready(Pen pen) {
        Source(pen.Color);
        cairo_set_line_width(cairo, (double)pen.Width);
    ...
    public void DrawLine(Pen pen, int x1, int y1, int x2, int y2) {
        if (pen.Style == PenStyle.None) { return; }
        double half = Ready(pen);
        cairo_new_path(cairo);
        cairo_move_to(cairo, (double)x1 + half, (double)y1 + half);
        cairo_line_to(cairo, (double)x2 + half, (double)y2 + half);
        cairo_stroke(cairo);
    }
    ...
    public void DrawRectangle(Pen pen, Rectangle bounds) {
        ...
        double half = Ready(pen);
        cairo_new_path(cairo);
        cairo_rectangle(cairo, (double)bounds.X + half, (double)bounds.Y + half,
                        (double)bounds.Width, (double)bounds.Height);
        cairo_stroke(cairo);
    }
    ...
    void Ellipse(Rectangle bounds) {
        cairo_new_path(cairo);
        cairo_save(cairo);
        cairo_translate(cairo, (double)bounds.X + (double)bounds.Width / 2.0,
                               (double)bounds.Y + (double)bounds.Height / 2.0);
        cairo_scale(cairo, (double)bounds.Width / 2.0, (double)bounds.Height / 2.0);
        cairo_arc(cairo, 0.0, 0.0, 1.0, 0.0, 6.283185307179586);
        cairo_restore(cairo);
    }
    ...
    void Path(Point[] points, double half, bool close) {
        cairo_new_path(cairo);
        if (points.Length == 0u) { return; }

        cairo_move_to(cairo, (double)points[0u].X + half, (double)points[0u].Y + half);
        for (nuint i = 1u; i < points.Length; i++) {
            cairo_line_to(cairo, (double)points[i].X + half, (double)points[i].Y + half);
        }
        if (close) { cairo_close_path(cairo); }
    }
    ...
    public void DrawString(String text, Font font, Color colour, int x, int y) {
        ...
        SelectFont(cairo, font);
        Source(colour);

        cairo_font_extents_t metrics;
        cairo_font_extents(cairo, &metrics);

        cairo_move_to(cairo, (double)x, (double)y + metrics.Ascent);
        cairo_show_text(cairo, text.ToPointer());
    ...
    void Decorate(String text, Font font, int x, int y, cairo_font_extents_t metrics) {
        if (!font.Underline && !font.Strikeout) { return; }
        ...
        if (font.Underline) {
            ...
            cairo_new_path(cairo);
            cairo_move_to(cairo, (double)x, at);
            cairo_line_to(cairo, (double)(x + extent.Width), at);
            cairo_stroke(cairo);
        }
        if (font.Strikeout) {
    ...
    public FSize MeasureString(String text, Font font) {
        ...
        cairo_font_extents_t metrics;
        cairo_font_extents(cairo, &metrics);
        if (text.IsEmpty()) { return Extent(0, (int)metrics.Height); }

        cairo_text_extents_t ink;
        cairo_text_extents(cairo, text.ToPointer(), &ink);
    ...
    public void DrawBitmap(IBitmapBackend picture, Point at) {
        ...
        cairo_save(cairo);
        gdk_cairo_set_source_pixbuf(cairo, (GdkPixbuf*)pixbuf,
    ...
    public void DrawBitmapIn(IBitmapBackend picture, Rectangle into) {
        ...
        cairo_save(cairo);
        cairo_new_path(cairo);
        cairo_rectangle(cairo, (double)into.X, (double)into.Y,
                        (double)into.Width, (double)into.Height);
        cairo_clip(cairo);

        cairo_translate(cairo, (double)into.X, (double)into.Y);
        cairo_scale(cairo, (double)into.Width / (double)width,
                           (double)into.Height / (double)height);
        gdk_cairo_set_source_pixbuf(cairo, pixbuf, 0.0, 0.0);
        cairo_paint(cairo);
        cairo_restore(cairo);
    }
}
