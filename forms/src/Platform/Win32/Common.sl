...
uint StaysPut() { return CcsNoResize | CcsNoParentAlign | CcsNoDivider; }
...
public class ToolBarPeer : ControlPeer, IToolBarPeer {
    ...
    public int AddButton(String text, int image, ToolButtonKind kind) {
        ...
        if (kind == ToolButtonKind.Separator) { button.Style = BtnsSeparator; }
        else if (kind == ToolButtonKind.Toggle) { button.Style = BtnsCheck; }
    ...
    int IndexOf(int command) {
        for (nuint i = 0u; i < commands.Count(); i += 1u) {
            if (commands.At(i) == command) { return (int)i; }
        }
        return -1;
    }

    int CommandAt(int index) {
        if (index < 0 || (nuint)index >= commands.Count()) { return -1; }
        return commands.At((nuint)index);
    }
    ...
    public void SetTextVisible(bool visible) {
        ...
        if (visible) { style = style | (long)TbStyleList; }
        else         { style = style & ~(long)TbStyleList; }
...
public class ProgressPeer : ControlPeer, IProgressPeer {
    ...
    public int GetValue() { return (int)SendMessageW(window, PbmGetPos, 0u, 0); }
    ...
    public void SetIndeterminate(bool indeterminate) {
        long style = Win32.User32.GetWindowLongPtrW(window, GwlStyle);
        if (indeterminate) { style = style | (long)PbsMarquee; }
        else               { style = style & ~(long)PbsMarquee; }
        Win32.User32.SetWindowLongPtrW(window, GwlStyle, style);
...
public class TrackBarPeer : ControlPeer, ITrackBarPeer {
    ...
    public int GetValue() { return (int)SendMessageW(window, TbmGetPos, 0u, 0); }
...
public class TabControlPeer : ControlPeer, ITabControlPeer {
    ...
    public int AddTab(String text, int image) {
        ...
        int at = (int)SendMessageW(window, TcmInsertItemW, (ulong)tabs,
                                   (long)(nuint)&item);
        tabs = tabs + 1;
        return at;
    }
    ...
    protected override bool NotifiedBy(int code, void* raw) {
        if (code != TcnSelChange) { return false; }
        IControlNotify? held = owning;
        if (held == null) { return false; }
        ((IControlNotify)held).OnPlatformValueChanged();
        return true;
...
public class TreeNodeHandle : ITreeNodeHandle {
    HTREEITEM item;
    public TreeNodeHandle(HTREEITEM handle) { item = handle; }
    public HTREEITEM Native() { return item; }
...
public class TreeViewPeer : ControlPeer, ITreeViewPeer {
    ...
    public ITreeNodeHandle AddNode(ITreeNodeHandle? parent, ITreeNodeHandle? previous,
        ...
        if (parent != null) {
            ...
            if (above is TreeNodeHandle real) { insert.Parent = real.Native(); }
        }
        if (previous != null) {
            ITreeNodeHandle before = (ITreeNodeHandle)previous;
            if (before is TreeNodeHandle real) { insert.InsertAfter = real.Native(); }
        }
    ...
    HTREEITEM NativeOf(ITreeNodeHandle node) {
        if (node is TreeNodeHandle real) { return real.Native(); }
        return null;
    }
    ...
    public String GetNodeText(ITreeNodeHandle node) {
        ...
        item.Children = 0;
        item.Param = 0u;
        if (SendMessageW(window, TvmGetItemW, 0u, (long)(nuint)&item) == 0) { return ""; }
    ...
    protected override bool NotifiedBy(int code, void* raw) {
        if (code != TvnSelChangedW) { return false; }
        IControlNotify? held = owning;
        if (held == null) { return false; }
        ((IControlNotify)held).OnPlatformValueChanged();
        return true;
...
public class ListViewPeer : ControlPeer, IListViewPeer {
    ...
    public void SetStyle(ListViewStyle style) {
        ...
        if (style == ListViewStyle.Details)        { was = was | (long)LvsReport; }
        else if (style == ListViewStyle.List)      { was = was | (long)LvsList; }
        else if (style == ListViewStyle.SmallIcon) { was = was | (long)LvsSmallIcon; }
    ...
    public int AddColumn(String text, int width, HorizontalAlignment alignment) {
        ...
        if (alignment == HorizontalAlignment.Right)       { column.Format = LvcfmtRight; }
        else if (alignment == HorizontalAlignment.Center) { column.Format = LvcfmtCenter; }
        column.Width = width;
        column.Text = wide.ToPointer();
        column.TextLength = 0;
        column.SubItem = columns;
        column.Image = 0;
        column.Order = 0;

        int at = (int)SendMessageW(window, LvmInsertColumnW, (ulong)columns,
                                   (long)(nuint)&column);
        columns = columns + 1;
        return at;
    }
    ...
    public void RemoveRow(int row) {
        if (SendMessageW(window, LvmDeleteItem, (ulong)row, 0) != 0 && rows > 0) {
            rows = rows - 1;
        }
    }
    ...
    public int RowCount() { return (int)SendMessageW(window, LvmGetItemCount, 0u, 0); }
    ...
    public void SetFullRowSelect(bool full, bool gridLines) {
        uint wanted = 0u;
        if (full) { wanted = wanted | LvsExFullRowSelect; }
        if (gridLines) { wanted = wanted | LvsExGridLines; }
    ...
    protected override bool NotifiedBy(int code, void* raw) {
        ...
        if ((details->Changed & 0x0008u) == 0u) { return false; }   // LVIF_STATE
        if ((details->NewState & LvisSelected) == 0u) { return false; }
        IControlNotify? held = owning;
        if (held == null) { return false; }
        ((IControlNotify)held).OnPlatformValueChanged();
        return true;
