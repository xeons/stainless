...
`struct { int a; long long b; }` is 16 bytes on Windows and 12 on Linux. The
compiler follows whichever the target is, and
[tests/cases/x86-wide-alignment](../tests/cases/x86-wide-alignment) checks both
against clang's `offsetof`.
...
offset 8   +----------------------+
           | weak    : nuint      |   weak count, +1 while the object lives
offset 16  +----------------------+
...
A **closure** is two words, and a struct like any other:

```
  offset 0            function   the method's address
  offset 8 (x86: 4)   receiver   the object it is called on, counted
```

The words are pointers, so the closure is sixteen bytes on a 64-bit target and
eight on x86. A call loads both and passes the receiver as argument zero, which
is where a method already expects `this` — so a bound method such as
`counter.Add` needs nothing generated, and `==` compares both words. Being two
fields is what gives it layout, both classifiers of §3.4 and reference counting
through the receiver, with nothing written for closures.

A function with no receiver — `names.Map(Upper)`, naming a module-level
function — has no `this` slot to be called with, so it gets one thunk per
function that takes the slot and discards it, and its receiver is null. One
thunk rather than a lambda at each use is what makes two closures over the same
function equal, so `-=` removes what `+=` added.

A **lambda** that captures is an ordinary class instance, generated by the
compiler, with one field per captured value and one method. Seen as a closure it
is that method beside that object; seen as a single-method interface it is the
object, and uses the interface dispatch of §2.8. Either way the object header
above and the ownership rules of §5 apply with nothing added. A lambda that
captures nothing may also become a delegate, which is its function alone.
...
What the linker sees is two methods and a field; the only record of the property
as such is the `SlPropertyInfo` entry a `[Reflect]` type carries (§2).
...
**`[NoUnknown]` takes the three away.** Some C libraries hand out the same shape
— a pointer to a vtable pointer — whose table does not begin with IUnknown;
XAudio2's voices are the case it was written for. On such an interface the first
method declared is slot 0, the call is the same two loads and an indirect call,
...
`Box<int>`, which a linker symbol may not contain, so every other ASCII
character in a qualified name becomes `_`, and a character past ASCII becomes
`_u` and its code point in four hex digits, so two names that differ still do.
...
`static readonly` reference is passed to `sl_make_immortal` as it is stored, so
it costs no reference traffic for the rest of the program and is never
